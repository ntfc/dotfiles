...
# Original: https://gist.github.com/mikeboiko/b6e50210b4fb351b036f1103ea3c18a9
...
do
    ...
    if [[ "${pane_process[1]}" == "zsh" || "${pane_process[1]}" == "bash" ]]; then
        # add an extra "Space Enter" so that the export command is not shown in
        # zsh history (only works if HIST_IGNORE_SPACE is set)
        tmux send-keys -t ${pane_process[0]} " export DISPLAY=$NEW_DISPLAY" Enter Space Enter
